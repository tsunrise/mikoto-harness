pub(crate) type Replacement = (usize, usize, Vec<String>);
...
impl SourceFile {
  ...
  pub(crate) fn line_texts(&self) -> Vec<String> {
    self.lines.iter().map(|line| line.text.clone()).collect()
  }

  pub(crate) fn apply_replacements(&mut self, replacements: &[Replacement]) {
    let mut source_lines = std::mem::take(&mut self.lines).into_iter();
    let mut new_lines = Vec::new();
    let mut source_index = 0;

    for (start_index, old_len, new_segment) in replacements {
      debug_assert!(*start_index >= source_index);
      for line in source_lines.by_ref().take(*start_index - source_index) {
        new_lines.push(line);
      }
      ...
      source_index = start_index + old_len;
    }

    new_lines.extend(source_lines);
    self.lines = new_lines;

    // Historical apply_patch behavior always leaves an updated file terminated.
...
mod tests {
  ...
  #[test]
  fn parses_all_supported_line_endings() {
    let file = SourceFile::parse("one\r\ntwo\nthree\rfour");
    assert_eq!(file.line_texts(), vec!["one", "two", "three", "four"]);
    assert_eq!(file.into_contents(), "one\r\ntwo\nthree\rfour");
  }
}
