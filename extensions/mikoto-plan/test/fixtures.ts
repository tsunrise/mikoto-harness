...
export function model(api: "openai-responses" | "openai-codex-responses" = "openai-codex-responses"): Model<Api> {
  return {
    ...
    streamSimple: (m, context, streamOptions) => {
      ...
      return stream(m as Model<"openai-codex-responses"> & Model<"openai-responses">, context, {
        ...
        fetch: async (input, init) => {
          ...
          return options.response ? options.response(requests.length) : sse();
        },
      });
    },
  });
...
export function modeItems(payload: any): any[] {
  return payload.input.filter((item: any) => Array.isArray(item.content)
    && item.content.some((block: any) => typeof block.text === "string"
      && block.text.startsWith("<developer_message>\n<collaboration_mode>")));
}
